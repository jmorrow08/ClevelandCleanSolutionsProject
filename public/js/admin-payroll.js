...
document.addEventListener("DOMContentLoaded", function () {
  console.log(
    "DEBUG: Admin Payroll Page script running (admin-payroll.js v1.7 - Added Client Claim Verification)."
  );
  ...
  const payrollActionMessageEl = document.getElementById(
  ...
  const addAdjustmentMessageEl = document.getElementById(
  ...
  if (
    ...
    console.error(
      "CRITICAL ERROR: Firebase SDK (app, auth, firestore, functions) not loaded correctly on admin-payroll.html."
    );
    if (payrollLoadingEl)
      payrollLoadingEl.textContent = "Error: Firebase SDK failed to load.";
    return;
  }
  ...
  try {
    ...
    console.error(
      "DEBUG: Error initializing Firebase services in admin-payroll.js:",
      error
    );
    if (payrollLoadingEl)
      payrollLoadingEl.textContent = "Error initializing Firebase services.";
    return;
  }
  ...
  async function fetchAndDisplayPayrollRecords() {
    ...
    if (!db) {
      ...
      if (payrollLoadingEl)
        payrollLoadingEl.textContent = "Error: DB not initialized.";
      return;
    }
    ...
    if (payrollActionMessageEl)
      showGeneralMessage(payrollActionMessageEl, "", "info");
    ...
    try {
      ...
      if (snapshot.empty) {
        ...
        snapshot.forEach((doc) => {
          ...
          const startDateStr =
            formatFirestoreTimestamp(payrollData.payPeriodStartDate) || "N/A";
          const endDateStr =
            formatFirestoreTimestamp(payrollData.payPeriodEndDate) || "N/A";
          const employeeName =
            payrollData.employeeName || payrollData.employeeId || "N/A";
          ...
          payrollHtml += `
                        <tr class="payroll-summary-row" data-payroll-id="${payrollId}">
                            ...
                            <td class="p-4"><span style="font-weight: bold; color: ${
                              isPaid ? "green" : "orange"
                            ...
                            <td class="p-4">
                                <div class="flex gap-2">
                                    ${
                                      !isPaid
                                        ? `<button class="action-button mark-payroll-paid-button" data-payroll-id="${payrollId}">Mark as Paid</button>`
                                        : ""
                                    }
          ...
          if (payrollData.jobs && payrollData.jobs.length > 0) {
            ...
            payrollData.jobs.forEach((job) => {
              payrollHtml += `<li class="job-item">
                                <span class="label">Job:</span> ${escapeHtml(
                                  job.locationName || "N/A"
                                )} on ${formatFirestoreTimestamp(
                job.serviceDate
              )}
                                <span class="label">Rate:</span> $${formatCurrency(
                                  job.rateApplied
                                )}
                                <span class="label">Earnings:</span> $${formatCurrency(
                                  job.earnings
                                )}
                                ${
                                  job.processedAt
                                    ? `<span class="label" style="font-size:0.8em; color:#777;">(Processed: ${formatFirestoreTimestamp(
                                        job.processedAt,
                                        {
                                          month: "short",
                                          day: "numeric",
          ...
          if (payrollData.adjustments && payrollData.adjustments.length > 0) {
            ...
            payrollData.adjustments.forEach((adj) => {
              payrollHtml += `<li class="adjustment-item">
                                <span class="label">Reason:</span> ${escapeHtml(
                                  adj.reason || "N/A"
                                )}
                                <span class="label">Amount:</span> $${formatCurrency(
                                  adj.amount
                                )}
                                ${
                                  adj.timestamp
                                    ? `<span class="label" style="font-size:0.8em; color:#777;">(Added: ${formatFirestoreTimestamp(
                                        adj.timestamp,
                                        {
                                          month: "short",
                                          day: "numeric",
                                ...
                                ${
                                  adj.adminUid
                                    ? `<span class="label" style="font-size:0.8em; color:#777;">(By Admin: ...${adj.adminUid.slice(
                                        -4
                                      )})</span>`
                                    : ""
                                }
  ...
  async function handleMarkAsPaidClick(payrollDocId, buttonElement) {
    ...
    if (!db || !serverTimestampFunction) {
      showGeneralMessage(
        payrollActionMessageEl,
        "Error: Database connection or timestamp function lost.",
        "error"
      );
      return;
    }
    if (
      !confirm(
        `Are you sure you want to mark payroll record ${payrollDocId} as 'Paid'?`
      )
    ) {
      return;
    }
    ...
    showGeneralMessage(
      payrollActionMessageEl,
      `Updating status for ${payrollDocId}...`,
      "info"
    );
    ...
    try {
      ...
      showGeneralMessage(
        payrollActionMessageEl,
        `Record ${payrollDocId} marked as Paid successfully! Refreshing list...`,
        "success"
      );
      setTimeout(() => {
      ...
      console.error(
        `Error updating payroll status for ${payrollDocId}:`,
        error
      );
      showGeneralMessage(
        payrollActionMessageEl,
        `Error updating status for ${payrollDocId}: ${error.message}`,
        "error"
      );
  ...
  async function populateEmployeeDropdownForAdjustments() {
    ...
    if (!db || !adjEmployeeSelect) {
      ...
      if (adjEmployeeSelect)
        adjEmployeeSelect.innerHTML = '<option value="">Error Loading</option>';
      return;
    }
    adjEmployeeSelect.disabled = true;
    adjEmployeeSelect.innerHTML =
    ...
    try {
      ...
      showGeneralMessage(
        addAdjustmentMessageEl,
        "Error loading employees for dropdown.",
        "error"
      );
    } finally {
  ...
  async function handleAddAdjustmentSubmit(event) {
    ...
    if (!auth || !auth.currentUser || !functions) {
      showGeneralMessage(
        addAdjustmentMessageEl,
        "Error: Firebase not ready or user not logged in.",
        "error"
      );
      return;
    }
    ...
    if (!employeeId || !payPeriodId || amountStr.trim() === "" || !reason) {
      showGeneralMessage(
        addAdjustmentMessageEl,
        "Employee, Pay Period ID, Amount, and Reason are required.",
        "error"
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        "Current user's custom claims from client-side:",
        idTokenResult.claims
      ); // Log the claims
      if (idTokenResult.claims.admin !== true) {
        showGeneralMessage(
          addAdjustmentMessageEl,
          "Client-side check: User does not have admin claim. Please re-login or contact support if this persists.",
          "error"
        );
      ...
      showGeneralMessage(
        addAdjustmentMessageEl,
        "Error verifying admin status on client. Please try again.",
        "error"
      );
      setFormDisabled(addAdjustmentForm, false);
      return;
    ...
    try {
      const addAdjustmentCallable = functions.httpsCallable(
      ...
      console.log(
        "DEBUG: Calling addPayrollAdjustment Cloud Function with payload:",
        payload
      );
      ...
      if (result.data && result.data.success) {
        showGeneralMessage(
          addAdjustmentMessageEl,
          result.data.message || "Adjustment added successfully!",
          "success"
        );
      ...
      console.error(
        "DEBUG: Error calling addPayrollAdjustment function:",
        error
      );
      ...
      if (error.code === "functions/not-found") {
        errorMessage =
          "Payroll adjustment service is not available. Please contact support.";
        ...
        errorMessage =
          "Could not add adjustment. Check function logs for details.";
      }
  ...
  function setupPayrollPageListeners() {
    ...
    if (payrollTableBodyEl) {
      ...
      payrollTableBodyEl.parentNode.replaceChild(
        newPayrollTableBodyEl,
        payrollTableBodyEl
      );
      ...
      payrollTableBodyEl.addEventListener("click", (event) => {
        ...
        if (targetButton.classList.contains("mark-payroll-paid-button")) {
          ...
          targetButton.classList.contains("view-payroll-details-button")
        ) {
          if (payrollId) {
            const detailsRow = document.getElementById(`details-${payrollId}`);
            if (detailsRow) {
              detailsRow.classList.toggle("visible");
              targetButton.textContent = detailsRow.classList.contains(
                "visible"
              )
                ? "Hide Details"
                : "View Details";
              ...
              console.warn(
                `Details row not found for payroll ID: ${payrollId}`
              );
            }
          } else {
      ...
      console.log(
        "DEBUG: Delegated listener attached to payroll table body for actions."
      );
    } else {
  ...
  auth.onAuthStateChanged((user) => {
    ...
    if (user) {
          ...
          db.collection("users")
            ...
            .then((docSnapshot) => {
              ...
              if (docSnapshot.exists) {
                ...
                console.log(
                  "DEBUG APR: User document data from Firestore:",
                  userData
                );
              } else {
                console.warn(
                  "DEBUG APR: User document not found in Firestore for UID:",
                  user.uid
                );
              }
              ...
              const isAdminByClaims =
                ...
                (claims.admin === true ||
                  ...
                  claims.standard_admin === true);
              const isAdminByRole =
                userData &&
                (userData.role === "admin" ||
                  userData.role === "super_admin" ||
                  userData.role === "standard_admin");

              if (isAdminByClaims || isAdminByRole) {
                console.log(
                  "DEBUG APR: Admin access confirmed for Payroll Page."
                );
                fetchAndDisplayPayrollRecords();
                ...
                console.error(
                  "DEBUG APR: Access denied. User does not have sufficient admin claims or role. Redirecting."
                );
            ...
            .catch((error) => {
              console.error(
                "DEBUG APR: Error fetching user data from Firestore:",
                error
              );
              ...
              if (isAdminByClaimsOnly) {
                console.warn(
                  "DEBUG APR: Proceeding with admin access based on claims despite Firestore user doc error."
                );
                fetchAndDisplayPayrollRecords();
                populateEmployeeDropdownForAdjustments();
                setupPayrollPageListeners();
      ...
      console.log(
        "DEBUG: Admin Payroll Page - No user signed in. Redirecting to login."
      );
      window.location.assign("/");
    }
  });
  ...
  window.populateEmployeeDropdownForAdjustments =
    populateEmployeeDropdownForAdjustments;
  window.fetchAndDisplayPayrollRecords = fetchAndDisplayPayrollRecords;
});
